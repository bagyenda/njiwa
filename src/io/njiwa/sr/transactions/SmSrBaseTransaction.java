...
public class SmSrBaseTransaction extends BaseTransactionType {
    ...
    @Override
    public Object sendTransaction(EntityManager em, Object xtr) throws Exception {
        ...
        SmSrBaseTransaction trObj = tr.getTransObject();
        List<byte[]> capdus = trObj.cAPDUs;
        int startIndex = trObj.index;
        ...
        if (canUseSMS && !hasMore) transportType = Transport.TransportType.SMS;
            ...
            sender = bipCatTP;
        } else transportType = Transport.TransportType.SMS; // Fall back. And hope for the best!
        ...
        if (ctx == null) throw new Exception("No supported transport mechanism");
        // Make the package
        ...
        if (xres.k != null && xres.k.length > 0)
            sres = sender.sendOTA(gwsession, otaParams, em, ctx, reqId, transid, transportType.toString(), TAG, xres.k);
        else {
        ...
        return new Utils.Triple<>(count, messageStatus, new Utils.Triple<>(nextSecs, reqId, sender.sendMethod()));
    }
}
